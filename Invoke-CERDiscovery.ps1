...
.DESCRIPTION
  Scopes:  Entra, Intune, Exchange, DNS, Teams, Azure   (cloud - run from your laptop, PowerShell 7 recommended)
           AD, Servers, Endpoints                        (on-prem - run on a domain-joined jump host / DC as domain admin, PS 5.1 ok)
           vSphere, Veeam, FortiGate                     (optional - need module / API access)
           Cloud = Entra,Intune,Exchange,DNS,Teams,Azure   OnPrem = AD,Servers   All = everything you supplied parameters for
...
$valid = 'Entra', 'Intune', 'Exchange', 'DNS', 'Teams', 'Azure', 'AD', 'Servers', 'Endpoints', 'vSphere', 'Veeam', 'FortiGate', 'Cloud', 'OnPrem', 'All'
...
foreach ($s in $Scope) {
    switch ($s) {
        ...
        'OnPrem' { 'AD', 'Servers' | ForEach-Object { $want.Add($_) } }
        'All' { 'Entra', 'Intune', 'Exchange', 'DNS', 'Teams', 'Azure', 'AD', 'Servers', 'Endpoints', 'vSphere', 'Veeam', 'FortiGate' | ForEach-Object { $want.Add($_) } }
        default { $want.Add($s) }
    }
}
$order = 'Entra', 'Intune', 'Exchange', 'DNS', 'Teams', 'Azure', 'AD', 'Servers', 'Endpoints', 'vSphere', 'Veeam', 'FortiGate'
$run = @($order | Where-Object { $want -contains $_ })
Write-Host ("CER-Discovery  client={0}  run={1}  output={2}  collectors={3}" -f $Client, $RunId, $OutputRoot, ($run -join ',')) -ForegroundColor Cyan
