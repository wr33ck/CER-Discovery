...
Invoke-CERSection -Collector $C -Section 'Lookups' -Script {
    ...
    Add-CEREvidence -Control 'M365-02' -Flag $(if ($bad.Count) { 'Attention' } else { 'OK' }) -Action 'Fix the SPF, DKIM and DMARC records for the domains that are not fully aligned. Target a single SPF record ending in -all with 10 or fewer lookups, DKIM signing on every sending domain including third-party senders, and DMARC moved through p=none to quarantine and then reject once the reports are clean. Domains that send no mail still need an SPF of v=spf1 -all and a DMARC reject policy, or they can be spoofed freely.' -Evidence ("DNS for {0} domains ({1} mail-enabled). Fully aligned (single SPF with -all/~all and <= 10 lookups, DKIM selector live, DMARC quarantine/reject with rua): {2}/{3}. Needs work: {4}. Parked/non-mail domains without null SPF + DMARC reject: {5} ({6}). Gateways: {7}.{8}" -f $results.Count, $mail.Count, ($mail.Count - $bad.Count - $failedLookups.Count), $mail.Count, (Join-CERList ($bad | ForEach-Object { "{0} [SPF {1} x{2}{7}, {3} lookups; DKIM {4}; DMARC p={5} rua={6}]" -f $_.Domain, $_.SPFAll, $_.SPFRecords, $_.SPFLookups, $(if ($_.DKIMSelectorsFound) { $_.DKIMSelectorsFound } else { 'none found' }), $_.DMARCPolicy, $_.DMARCReporting, $(if ($_.SPFMalformed) { ' MALFORMED: v=spf1 is not the first/only string in its TXT record (RFC 7208 concatenation breaks it)' } else { '' }) }) 6), $parkedBad.Count, (Join-CERList ($parkedBad | ForEach-Object { $_.Domain }) 5), (Join-CERList ($mail | Group-Object Gateway | ForEach-Object { "{0}={1}" -f $_.Name, $_.Count }) 4), $(if ($failedLookups.Count) { " TXT lookup failed (resolver/TCP issue) for: " + (($failedLookups | ForEach-Object { $_.Domain }) -join ', ') + " - re-run from another network." } else { '' }))
    $noCaa = @($results | Where-Object { -not $_.CAA }); $dnssec = @($results | Where-Object DNSSEC)
    Add-CEREvidence -Control 'NET-10' -Flag $(if ($noCaa.Count -eq $results.Count) { 'Attention' } else { 'Info' }) -Action 'Add CAA records so only the intended certificate authorities can issue for these domains, confirm registrar lock is on and expiry is more than six months out, and consider DNSSEC and MTA-STS. Domain and DNS control is the root of every other control - losing the registrar account loses the mail, the certificates and the identity provider all at once.' -Evidence ("Domain hygiene: NS providers {0}; CAA record present on {1}/{2}; DNSSEC signed {3}/{2}; MTA-STS {4}, TLS-RPT {5}. Registrar lock and expiry dates: check TPP Wholesale / whois (manual)." -f (Join-CERList ($results | Group-Object NSProvider | ForEach-Object { "{0}={1}" -f $_.Name, $_.Count }) 4), ($results.Count - $noCaa.Count), $results.Count, $dnssec.Count, @($results | Where-Object MTASTS).Count, @($results | Where-Object TLSRPT).Count)
    ...
    if ($onprem.Count -or $mxOnprem.Count) { Add-CEREvidence -Control 'M365-05' -Flag Attention -Action 'Autodiscover or MX pointing at a non-Microsoft host usually means an on-prem Exchange server still published to the internet. Confirm what is actually listening on those names and run the ExchangeOnPrem collector against it - a forgotten internet-facing Exchange server on an unsupported build is the highest-severity finding this toolkit produces.' -Evidence ("Autodiscover/MX pointing at non-Microsoft/non-gateway hosts (possible on-prem Exchange still published): {0}" -f (Join-CERList (($onprem | ForEach-Object { "{0} -> {1}" -f $_.Domain, $_.Autodiscover }) + ($mxOnprem | ForEach-Object { "{0} MX {1}" -f $_.Domain, $_.MX })) 5)) }
}
