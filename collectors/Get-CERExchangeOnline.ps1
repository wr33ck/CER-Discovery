...
Invoke-CERSection -Collector $C -Section 'OrgConfigAudit' -Script {
    ...
    Add-CEREvidence -Control 'M365-04' -Flag $flag -Action 'Turn on mailbox auditing and unified audit log ingestion if either is off, and disable SMTP AUTH org-wide with per-mailbox exceptions for the devices that genuinely need it. Without the unified audit log there is no record to investigate with - and it cannot be applied retrospectively to events that have already happened.' -Evidence ("Exchange Online org '{0}': mailbox auditing default {1} (AuditDisabled={2}); unified audit log ingestion {3}; Modern auth (OAuth2ClientProfileEnabled) {4}; org-wide SMTP AUTH disabled {5}; default authentication policy '{6}'." -f $oc.Name, $(if ($oc.AuditDisabled) { 'OFF' } else { 'on' }), $oc.AuditDisabled, $aal.UnifiedAuditLogIngestionEnabled, $oc.OAuth2ClientProfileEnabled, $tc.SmtpClientAuthenticationDisabled, $oc.DefaultAuthenticationPolicy)
    Add-CEREvidence -Control 'IAM-04' -Flag $(if ($tc.SmtpClientAuthenticationDisabled) { 'OK' } else { 'Attention' }) -Action 'Disable SMTP AUTH at the organisation level and enable it only on the specific mailboxes that need it, then turn off IMAP and POP in the CAS mailbox plans. These are the paths that bypass Conditional Access entirely, which is why password spraying targets them.' -Evidence ("EXO: SMTP AUTH disabled org-wide = {0}; authentication policies: {1}; CAS mailbox plans allow IMAP/POP by default: {2}." -f $tc.SmtpClientAuthenticationDisabled, (Join-CERList ($authPol | ForEach-Object { "{0} (basic ActiveSync={1}, IMAP={2}, POP={3}, SMTP={4})" -f $_.Name, $_.AllowBasicAuthActiveSync, $_.AllowBasicAuthImap, $_.AllowBasicAuthPop, $_.AllowBasicAuthSmtp }) 3), (Join-CERList ($plans | ForEach-Object { "{0}: IMAP={1} POP={2}" -f $_.Name, $_.ImapEnabled, $_.PopEnabled }) 3))
    Add-CEREvidence -Control 'M365-05' -Flag $(if ($hybrid.Count) { 'Attention' } else { 'Info' }) -Action 'A hybrid configuration object means an on-prem Exchange server exists or once did. Run the ExchangeOnPrem collector to establish which, and whether it still holds mailboxes. Hybrid objects left behind after a completed migration keep a decommissioned server''s configuration alive in the tenant.' -Evidence ("Hybrid configuration objects in EXO (Get-OnPremisesOrganization): {0} ({1}); intra-org connectors: {2}. Hybrid present means an on-prem Exchange server still exists (or was never cleaned up)." -f $hybrid.Count, (Join-CERList ($hybrid | ForEach-Object { $_.OrganizationName }) 3), $ioc.Count)
    Add-CEREvidence -Control 'M365-02' -Flag Info -Action 'Check every accepted domain is still in use and correctly typed. An internal relay domain that should be authoritative silently sends mail out to the internet rather than delivering it locally, and a domain nobody uses is one an attacker can send as.' -Evidence ("Accepted domains: {0} ({1} authoritative, {2} internal relay). DNS checks per domain are in the DNS collector." -f $script:acceptedDomains.Count, @($script:acceptedDomains | Where-Object { $_.DomainType -eq 'Authoritative' }).Count, @($script:acceptedDomains | Where-Object { $_.DomainType -eq 'InternalRelay' }).Count)
}
...
Invoke-CERSection -Collector $C -Section 'DkimForwardingRules' -Script {
    ...
    Add-CEREvidence -Control 'M365-02' -Flag $(if ($dkimOff.Count) { 'Attention' } else { 'OK' }) -Action 'Enable DKIM signing for the authoritative custom domains that lack it. Without DKIM, DMARC has only SPF to align against, so forwarded mail fails and legitimate senders end up quarantined - which is what usually forces DMARC back to p=none.' -Evidence ("M365 DKIM signing enabled for {0}/{1} authoritative custom domains; not enabled: {2}. (Mimecast or other gateway DKIM is checked via DNS selectors.)" -f ($auth.Count - $dkimOff.Count), $auth.Count, (Join-CERList ($dkimOff | ForEach-Object { $_.DomainName }) 6))
    $fwdOpen = @($osp | Where-Object { $_.AutoForwardingMode -ne 'Off' }); $rdOpen = @($rd | Where-Object { $_.AutoForwardEnabled })
    Add-CEREvidence -Control 'M365-04' -Flag $(if ($fwdOpen.Count -or $extForward.Count -or $sclBypass.Count) { 'Attention' } else { 'OK' }) -Action 'Set the outbound spam policy auto-forwarding mode to Off, disable auto-forward on remote domains, and review the transport rules that redirect, BCC or copy mail. External auto-forwarding is the standard exfiltration path after a mailbox compromise, and it survives a password reset.' -Evidence ("Auto-forwarding: outbound spam policies with AutoForwardingMode <> Off: {0} ({1}); remote domains allowing auto-forward: {2} ({3}); transport rules that redirect/BCC/copy: {4} ({5}); transport rules setting SCL -1 (spam bypass): {6} ({7}); total rules {8} ({9} enabled)." -f $fwdOpen.Count, (Join-CERList ($fwdOpen | ForEach-Object { "{0}={1}" -f $_.Name, $_.AutoForwardingMode }) 3), $rdOpen.Count, (Join-CERList ($rdOpen | ForEach-Object { $_.DomainName }) 3), $extForward.Count, (Join-CERList ($extForward | ForEach-Object { $_.Name }) 4), $sclBypass.Count, (Join-CERList ($sclBypass | ForEach-Object { $_.Name }) 4), $rules.Count, @($rules | Where-Object { $_.State -eq 'Enabled' }).Count)
...
Invoke-CERSection -Collector $C -Section 'Mailboxes' -Script {
    ...
    Add-CEREvidence -Control 'M365-04' -Flag $flag -Action 'Remove mailbox forwarding to external addresses unless each one is documented and approved, block sign-in on shared mailboxes, and turn on inbox rule auditing. External forwarding and hidden inbox rules are the two things attackers set up first and clients discover last.' -Evidence ("Mailboxes: {0} user, {1} shared, {2} room/equipment. Mailbox-level forwarding: {3}, to external domains: {4} ({5}). Shared mailboxes with sign-in still enabled: {6} ({7}){8}. Inactive user mailboxes (> {9} days, of {10} checked): {11} ({12}). Inbox rules forwarding externally: {13}{14}." -f $user.Count, $shared.Count, $room.Count, $fwd.Count, $fwdExt.Count, (Join-CERList ($fwdExt | ForEach-Object { $_.PrimarySmtpAddress }) 5), $sharedEnabled.Count, (Join-CERList $sharedEnabled 5), $(if (-not $entraUsers) { ' [Entra user data not in this run - sign-in state unknown]' } else { '' }), $StaleDays, $checked, $inactiveOld.Count, (Join-CERList ($inactiveOld | ForEach-Object { $_.Mailbox }) 5), $rulesExt.Count, $(if (-not $CheckInboxRules) { ' [not checked - use -CheckInboxRules]' } else { " (of $rulesChecked mailboxes)" }))
    Add-CEREvidence -Control 'M365-08' -Flag Info -Action 'Confirm the retention and hold position matches what the client is actually obliged to keep, and turn mailbox auditing on where it is explicitly off. Litigation hold and online archive are also licence-dependent, so check entitlement matches deployment.' -Evidence ("Litigation hold on {0} mailboxes; online archive active on {1}/{2} user mailboxes; mailbox auditing explicitly off on {3}." -f $lit.Count, $archive.Count, $user.Count, $auditOff.Count)
    if ($sharedLicensed.Count) { Add-CEREvidence -Control 'M365-10' -Flag Attention -Action 'Remove the licences from shared mailboxes that do not need them - a shared mailbox only needs a licence for an online archive, a litigation hold, or to exceed 50 GB. This is usually the fastest licence saving available in the tenant.' -Evidence ("{0} shared mailboxes carry a licence (only needed for archive/hold or > 50 GB): {1}" -f $sharedLicensed.Count, (Join-CERList $sharedLicensed 5)) }
    if ($inactiveOld.Count) { Add-CEREvidence -Control 'M365-10' -Flag Attention -Action 'Review the inactive licensed mailboxes for leavers who were never offboarded, then either reclaim the licence or convert to a shared mailbox. Each one is both an unnecessary cost and an unmonitored account with a mailbox.' -Evidence ("{0} user mailboxes inactive > {1} days (candidates for licence removal / conversion to shared)." -f $inactiveOld.Count, $StaleDays) }
}
...
Invoke-CERSection -Collector $C -Section 'MobileAndSharing' -Script {
    ...
    Add-CEREvidence -Control 'END-14' -Flag Info -Action 'Where Intune app protection and Conditional Access are in place, they supersede the Exchange mobile device mailbox policy - confirm which is actually governing mobile access rather than assuming the older policy still applies. Two overlapping controls with different settings usually means neither is what anyone intended.' -Evidence ("Exchange mobile device mailbox policy (default '{0}'): password required={1}, min length={2}, non-provisionable devices allowed={3}, inactivity lock={4}, device encryption={5}. Intune app protection/CA supersedes this where present." -f $(if ($def) { $def.Name } else { 'n/a' }), $(if ($def) { $def.PasswordEnabled } else { '' }), $(if ($def) { $def.MinPasswordLength } else { '' }), $(if ($def) { $def.AllowNonProvisionableDevices } else { '' }), $(if ($def) { $def.MaxInactivityTimeLock } else { '' }), $(if ($def) { $def.DeviceEncryptionEnabled } else { '' }))
    Add-CEREvidence -Control 'M365-06' -Flag Info -Action 'Review the OWA policy settings for third-party file providers and additional storage providers, and set the calendar sharing policy domains deliberately. These are the routes corporate content leaves the tenant through Outlook on the web without touching an endpoint.' -Evidence ("OWA policies: third-party file providers enabled on {0}/{1}; additional storage providers on {2}; calendar sharing policy domains: {3}." -f @($owa | Where-Object ThirdPartyFileProvidersEnabled).Count, $owa.Count, @($owa | Where-Object AdditionalStorageProvidersAvailable).Count, (Join-CERList (($sp | Where-Object Default).Domains) 4))
}
...
if (-not $SkipPurview) {
    Invoke-CERSection -Collector $C -Section 'Purview' -Script {
        ...
        Add-CEREvidence -Control 'M365-08' -Flag $flag -Action 'Confirm retention policies cover Exchange, SharePoint, OneDrive and Teams, and move the DLP policies from test mode to enforcement with a date. A DLP policy in test mode reports and blocks nothing; for AU clients the baseline worth starting from is TFN, credit card and personal information.' -Evidence ("Purview: retention policies {0} enabled ({1}); DLP policies {2} (enforced {3}, test mode {4}: {5}); sensitivity labels {6} ({7} published via {8} policies); retention labels {9}." -f @($ret | Where-Object Enabled).Count, (Join-CERList $retTxt 4), $dlp.Count, $dlpEnforce.Count, $dlpTest.Count, (Join-CERList ($dlp | ForEach-Object { $_.Name }) 4), $labels.Count, (($labelPol | ForEach-Object { @($_.Labels).Count } | Measure-Object -Sum).Sum), $labelPol.Count, $retLabels.Count)
    }
}
