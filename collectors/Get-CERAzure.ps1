...
Invoke-CERSection -Collector $C -Section 'Subscriptions' -Script {
    ...
    if ($script:subs.Count -eq 0) { Add-CEREvidence -Control 'AZ-01' -Flag Info -Evidence 'No enabled Azure subscriptions visible to this account (no Azure footprint, or Lighthouse delegation missing).'; Set-CERSectionResult -Status Skipped -Note 'no subscriptions' }
}
...
Invoke-CERSection -Collector $C -Section 'Inventory' -Script {
    ...
    Add-CEREvidence -Control 'AZ-01' -Flag $flag -Evidence ("Azure: {0} subscriptions ({1}); {2} resources across {3} types; tag coverage owner {4}, cost-centre {5}, environment {6}; resource locks {7}; management groups visible {8}; Lighthouse registration assignments {9}." -f $subs.Count, (Join-CERList ($subs | ForEach-Object { $_.Name }) 4), ($res | Measure-Object count_ -Sum).Sum, $res.Count, (ConvertTo-CERPct $t.owner $t.total), (ConvertTo-CERPct $t.cc $t.total), (ConvertTo-CERPct $t.env $t.total), $locks.Count, $mg.Count, $lighthouse.Count)
}
